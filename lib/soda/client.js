 ...
 *
 */
var getCommonPrefixes = function(cmd) {
  return [
      'assert' + cmd
    , 'assertNot' + cmd
    , 'verify' + cmd
...
exports.accessors.map(function(cmd){
  var accessorCommands = ['get'+cmd].concat(getCommonPrefixes(cmd));
  accessorCommands.map(function(accessorCommand) {
    exports.commands.push(accessorCommand);
  });
});

/**
 * The same goes for booleanAccessors.
 *
 * In this case, however, the correct command
 * is "is"+cmd, not "get"+cmd
 *
 */

exports.booleanAccessors.map(function(cmd){
  var accessorCommands = ['is'+cmd].concat(getCommonPrefixes(cmd));
  accessorCommands.map(function(accessorCommand) {
    exports.commands.push(accessorCommand);
  });
});
